...
Apache Cordova is a open source platform to build Native Mobile Applicatons using HTML, CSS and JavaScript. PhoneGap is Adobe's productised implementation of Cordova. 

Manatee Works provides the PhoneGap plugin via NPM, but also as a Git repository, so both paths are viable for integration.
This plugin is the successor of [manateeworks-barcodescanner-v3 plugin](https://www.npmjs.com/package/manateeworks-barcodescanner-v3)
...
```ssh
phonegap plugin add mwsdk-phonegap-plugin
```

or   

```ssh
phonegap plugin add https://github.com/Cognex-ID/mwsdk-phonegap-plugin.git
```
or   
...
The Manatee Works Barcode Scanner SDK requires a license to work properly. A [Manatee Works Barcode Scanner SDK Evaluation License](https://cmbdn.cognex.com/lpr/evaluation) is available for a free 30-day trial. Be sure to select each platform, symbology, and parser your app will support.
...
where we replace the variables VALID_ANDROID_KEY, VALID_IOS_KEY with their respective values from the MWDN license section.
